...
class UntestedBloodList
{
    var list: array<UntestedBlood>;
    var upto: int;
    ...
    // Correspond to UntestedBloodList.py::addBlood() -- inherited from BloodList
    method addBlood(blood: UntestedBlood)
    ...
    ensures list == old(list) || fresh(list);
    {
        ...
        if upto == list.Length
        {
            var newlist := new UntestedBlood[2*list.Length];
    ...
    // Correspond to UntestedBloodList.py::getBlood() -- inherited from BloodList
    method getBlood(id: int) returns (blood: UntestedBlood)
    requires Valid(); ensures Valid();
    requires UniqueId(); ensures UniqueId();
    ...
    // Correspond to UntestedBloodList.py::removeBlood() -- inherited from BloodList
    method removeBlood(blood: UntestedBlood)
    requires Valid(); ensures Valid();
    requires UniqueId(); ensures UniqueId();
    ...
    // Correspond to UntestedBloodList.py::extractBlood() -- inherited from BloodList
    method extractBlood(id: int) returns (blood: UntestedBlood)
    requires Valid(); ensures Valid();
    requires UniqueId(); ensures UniqueId();
